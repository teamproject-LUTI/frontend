...
const QnaWrite = () => {
    ...
    // AuthContext에서 userId, token 꺼내기
    ...
    // askId 추출 함수
    const extractAskId = (responseData) => {
        console.log('문의 저장 응답:', responseData);
        ...
        if (responseData.data && typeof responseData.data === 'object') {
            ...
            askId = responseData.data.id || responseData.data.askId || responseData.data.ask_id;
        } else if (typeof responseData.data === 'number') {
            // data가 숫자(ID)인 경우
            askId = responseData.data;
        } else if (responseData.id) {
            // 직접 id 필드가 있는 경우
            askId = responseData.id;
        } else if (responseData.askId) {
            // askId 필드가 있는 경우
            askId = responseData.askId;
        }

        console.log('추출된 askId:', askId);
        return askId;
    };

    // 문의글 저장 함수
    const saveAsk = async (title, contentHtml) => {
        console.log('문의글 저장 요청:', { title, content: contentHtml });
    ...
    // 첨부파일 업로드 함수
    const uploadAttachments = async (askId, files) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // 1) 문의글 본문 저장
            const askRes = await saveAsk(title, contentHtml);

            // 2) askId 추출
            const askId = extractAskId(askRes.data);

            if (!askId) {
                console.error('askId를 찾을 수 없습니다:', askRes.data);
                throw new Error('문의글 저장은 완료되었지만 ID를 확인할 수 없습니다.');
            }

            // 3) 첨부파일 업로드 (있을 경우에만)
            if (attachments.length > 0) {
                await uploadAttachments(askId, attachments);
            }

            // 성공 메시지 표시
            alert('문의글이 성공적으로 저장되었습니다.');
